  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        target_audience: {
          locations: form.target_audience.locations.split(',').map(v => v.trim()).filter(Boolean),
          age_range: form.target_audience.age_range,
          interests: form.target_audience.interests.split(',').map(v => v.trim()).filter(Boolean),
        },
      };
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={s.card}>
          ...
          <div style={s.platformGrid}>
            {platforms.map(p => (
              <button key={p.id} type="button" style={s.platformBtn(form.platforms.includes(p.id), p.color)} onClick={() => togglePlatform(p.id)}>
        ...
        <div style={s.card}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <input style={s.input} value={form.target_audience.locations} onChange={setAud('locations')} placeholder="US, UK, CA, AU" onFocus={focusBorder} onBlur={blurBorder} />
          </div>
          <div style={s.row}>
            <div>
              <label style={s.label}>Age Range</label>
              <input style={s.input} value={form.target_audience.age_range} onChange={setAud('age_range')} placeholder="18-45" onFocus={focusBorder} onBlur={blurBorder} />
            </div>
            <div>
              <label style={s.label}>Interests (comma separated)</label>
              <input style={s.input} value={form.target_audience.interests} onChange={setAud('interests')} placeholder="technology, fashion, sports" onFocus={focusBorder} onBlur={blurBorder} />
            </div>
          </div>
        </div>
